...
${req.valuation.estimates.map((x) => `  - ${x.label}: fair value ${x.fairValue ?? "n/a"}, upside ${x.upsidePercent ?? "n/a"}%, formula: ${x.formula}`).join("\n")}` : "- none";
  const macro = req.macroOverlays?.length ? req.macroOverlays.map((series) => {
    ...
    return `- ${series.label}: ${last2 ? `${last2.value} ${series.unit}` : "n/a"} (${series.sourceName})`;
...
function registerIpcHandlers() {
  ...
  import_electron4.ipcMain.handle(IPC.signalsScan, async (_e, rawRequest) => {
    const request = cleanSignalScanRequest(rawRequest);
    try {
